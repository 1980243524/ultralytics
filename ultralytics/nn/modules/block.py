...
class EMA(nn.Module):
    ...
    def __init__(self, c1, c2, factor=8):
        ...
        self.conv_match = nn.Identity() if c1 == c2 else Conv(c1, c2, 1, 1)
        assert c2 % self.groups == 0, f"Channel size {c2} should be divisible by groups {self.groups}"
        
        self.softmax = nn.Softmax(-1)
        self.agp = nn.AdaptiveAvgPool2d((1, 1))
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))
        self.gn = nn.GroupNorm(c2 // self.groups, c2 // self.groups)
        self.conv1x1 = nn.Conv2d(c2 // self.groups, c2 // self.groups, kernel_size=1, stride=1, padding=0)
        self.conv3x3 = nn.Conv2d(c2 // self.groups, c2 // self.groups, kernel_size=3, stride=1, padding=1)
...
class RepC3_EMA(RepC3):
    """Rep C3 with EMA attention."""
